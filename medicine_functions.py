...
def add_medicine():
    if (os.path.isfile('medicine.csv')==False):
...
#         writer.writeheader()
        ...
        if quantity >min_quantity:
            to_pur = 0
        writer.writerow({'medi_name':medi_name,'med_id':med_id,'sale':sale,'unit':unit,'quantity':quantity,'min_quantity':min_quantity,'comp_name':comp_name,'sup_id':sup_id,'to_pur':to_pur})
        if (os.path.isfile('purchase.csv')==False):
            with open ('purchase.csv','w') as cfile:
                ...
                writer.writeheader()
        with open('purchase.csv','a+') as cfile:
